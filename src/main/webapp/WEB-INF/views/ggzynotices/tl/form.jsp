	...
	<div class="div_xmtj">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tble_xmlb">
			...
			<tr>
				...
				<td class="td2" colspan="3">${ggzyBidPro.tenderee.subjectName }</td>
			...
			<tr>
				...
				<td class="td2"><mh:lookUpTree rootId="18" group="noticeNature" value="${ggzyNotice.noticeNature }" defaultId="21720" defaultName="正常公告"/></td>
			...
			<tr>
				<td class="td1"><label><spring:message code="ggzynotice_tl_tendererdocgaintime" />：</label></td>
				...
				<td class="td1"><label><spring:message code="ggzynotice_tl_tendererdocgainendtime" />：</label></td>
			...
			<tr>
				<td class="td1"><label>开标时间：</label></td>
				<td class="td2"><input type="text" name="bidDocSubEndtime" class="date validate[required] required" value='<spring:eval expression="ggzyNotice?.bidDocSubEndtime"/>' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd 09:30:00', minDate:'#F{$dp.$D(\'tendererDocGainEndTime\',{d:20, H:-%H-1})||\'%y-%M-{%d+20} 09:30:00\'}'})" readonly="readonly" /></td>
				<td class="td1"><label><spring:message code="ggzynotice_biddocsubway" />：</label></td>
				<td class="td2">
				<input name="bidDocSubWay.id" value="${ggzyNotice.bidDocSubWay.id}" type="hidden"/>
				<input name="bidDocSubWay.content" value="${mh:clob2String(ggzyNotice.bidDocSubWay.content) }" class="validate[required] required" /></td>
			</tr>
			<tr>
