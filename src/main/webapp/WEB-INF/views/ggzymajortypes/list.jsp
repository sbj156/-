...
<dwz:paginationForm
	action="${contextPath }/${business_ctx}/ggzymajortypes" page="${page }"
		...
		<ul class="toolBar">
			<shiro:hasPermission name="User:save">
				<li><a iconClass="application_add" target="dialog"
					...
					href="${contextPath }/${business_ctx}/ggzymajortypes?form"><span><spring:message code="ggzymajortype_addbtn"/> </span></a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="User:edit:User拥有的资源">
				<li><a iconClass="application_edit" target="dialog"
					...
					href="${contextPath }/${business_ctx}/ggzymajortypes/{slt_uid}?form"><span><spring:message code="ggzymajortype_updatebtn"/></span></a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="User:delete:User拥有的资源">
				<li><a iconClass="application_delete" target="selectedTodo" rel="ids"
					href="${contextPath }/${business_ctx}/ggzymajortypes/delete"
					title="确认要删除?"><span><spring:message code="ggzymajortype_deletebtn"/> </span></a></li>
			</shiro:hasPermission>
	...
	<table class="table" layoutH="137" width="100%">
		<thead>
			<tr>
				...
				<th width="100" orderField="code" class="${page.orderField eq 'code' ? page.orderDirection : ''}"><spring:message code="ggzybusinesstype_code" /></th>
			</tr>
		</thead>
		<tbody>
			<c:forEach var="item" items="${list}">
				<tr target="slt_uid" rel="${item.majorTypeId}">
					<td><input name="ids" value="${item.majorTypeId}" type="checkbox"></td>
					<td>${item.name}</td>
					<td>${item.code}</td>
				</tr>
			</c:forEach>
