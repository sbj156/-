...
<form method="put" action="${contextPath }/${business_ctx}/${module_path}" class="required-validate pageForm" onsubmit="return validateCallback(this, dialogReloadNavTab);">
	...
	<div class="pageFormContent" layoutH="58">
		...
		<p>
			...
			<input type="text" name="represtative" value="${ggzyObjection.represtative }" class="input-medium validate[required,maxSize[50]] required" maxlength="128"/>
		</p>
		<p>
			<label>受理时间：</label>
			<input type="text" name="processingTime" value="<fmt:formatDate value="${ggzyObjection.processingTime.time}" pattern="yyyy-MM-dd"/>" class="input-medium validate[required,maxSize[32]] required" maxlength="32"/>
		</p>		
		<p>
			<label>处理结果：</label>
		...
		<p>
			<label>反馈时间：</label>
			<input type="text" name="feedbackTime" value="<fmt:formatDate value="${ggzyObjection.feedbackTime.time }" pattern="yyyy-MM-dd"/>" class="input-medium validate[custom[phone],maxSize[32]]" maxlength="32"/>
		</p>		
		<p>
			<label>异议与投诉内容：</label>
			<input type="text" name="objectionContent" value="${mh:clob2String(ggzyObjection.objectionContent)}" class="input-medium validate[maxSize[100]]" maxlength="128"/>
		</p>
	</div>
