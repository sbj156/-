...
function validApply(time_){
	...
	var data = $.parseJSON( $('#selectDataapplynoticetable', navTab.getCurrentPanel()).val());
	...
	<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a iconClass="application_edit" target="dialog"
				rel="apply_add" width="800" height="600"
				href="${contextPath }/${business_ctx}/${module_path}?form&noticeId={slt_uid}" name="ggzyapply_addbtn" click="{url:'${contextPath }/app/sysdate',fun:validApply}"><span><spring:message
							code="ggzyapply_addbtn"/></span></a></li>
			</shiro:hasPermission>
			...
			<li><a iconClass="application_edit" target="navTab"
				...
				href="${contextPath }/${business_ctx}/${module_path}?nv_=ggzyapplys/applylist&search_EQ_notice.noticeId={slt_uid}&orderField=applyTime&businessTypeId=${param.businessTypeId}" name="ggzynotice_updatebtn"><span><spring:message
							code="ggzyapply_showlistbtn" /></span></a></li>
			</shiro:hasPermission>
		</ul>
	</div>
	...
	<table id="applynoticetable" class="table" layoutH="137" width="100%">
		<thead>
			<tr>
				...
				<th width="70"><spring:message code="ggzynotice_tendererdocgaintime" /></th>
				<th width="70"><spring:message code="ggzynotice_tendererdocgainendtime" /></th>
			</tr>
		</thead>
		<tbody>
