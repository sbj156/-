...
<form id="permissionForm" method="post" action="${contextPath }/management/security/module/update" class="required-validate pageForm" onsubmit="">
	...
	<div id="permissionFormContent" class="pageFormContent" layoutH="58">
		...
		<p>
			<label>手机显示名称：</label>
			<input type="text" name="pdaName" class="input-medium validate[maxSize[64]]" maxlength="64" value="${module.pdaName }"/>
		</p>	
		<p>
			<label>优先级：</label>
		...
		<p>
			<label>显示父节点名称：</label>
			<select name="showParent">
				<option value="0" ${module.showParent?"":"selected" }>不显示</option>
				<option value="1" ${module.showParent?"selected":"" }>显示</option>
			</select>
		</p>
		<p>
			<label>是否是业务模块：</label>
			<input type="radio" value="0" name="businessModule" ${module.businessModule? "":"checked" }/>否
			<input type="radio" value="1" name="businessModule" ${module.businessModule? "checked":"" }/>是
		</p>
		<p>
			<label>是否是JSON参数：</label>
			<input type="radio" value="0" name="jsonParam" ${module.jsonParam? "":"checked" }/>否
			<input type="radio" value="1" name="jsonParam" ${module.jsonParam? "checked":"" }/>是
		</p>
		<p class="nowrap">
			<label>链接参数：</label>
			<textarea name="parameters" cols="29" rows="3" maxlength="256" class="input-medium textarea-scroll">${module.parameters }</textarea>
		</p>
		<p>
		...
		<legend>动态新增</legend>
			...
			<p>
				<label>操作名称：</label>
				<input type="text" name="_sn" class="input-medium validate[required,maxSize[32],minSize[3]] required" maxlength="32"/>
			</p>
			<p>
				<label>描述：</label>
