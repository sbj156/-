...
<c:set value="${contextPath }/${business_ctx}/analysis/classify/majorbid20" var="listUrl_" />
<dwz:paginationForm action="${listUrl_ }" page="${page }" onsubmit="return navTabSearch(this)">
	<div class="pageHeader">
		<div class="searchBar">
			<ul class="searchContent">
				<li><label>业务类型：${businessType.name }</label>
				<li><label>专业类型：</label>
				<select class="combox" name="major_type_id">
					<c:forEach items="${businessType.ggzyBusinessTypeMajors }" var="item">
					<option value="${item.majorType.majorTypeId}" ${param.major_type_id eq item.majorType.majorTypeId? "selected":"" }>${item.majorType.name }</option>
					</c:forEach>
				</select>
				</li>
