...
<c:set value="${contextPath }/${business_ctx}/analysis/overall/expertamount" var="listUrl_" />
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tble_xmlb">
			...
			<c:forEach items="${expert}" var="item" varStatus="status">
				<tr>
					...
					<td class="td1"><a target="navTab" rel="expert_amount_${item.business_type_id }" href="${contextPath }/${business_ctx}/analysis/overall/businessexpert?business_type_id=${item.business_type_id }">${item.name }<span style="display: none">专家参与次数</span></a></td>
