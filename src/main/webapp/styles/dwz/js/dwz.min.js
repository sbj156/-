	...
	if (WdatePicker){
		$('input.date', $p).each(function(){
			...
			/*if($this.attr("datePicker"))
				$.extend(opts, eval("(" + $this.attr("datePicker")+")"));
			$this.bind("focus", opts, WdatePicker);
			console.info(opts);*/
//			$this.attr("onfocus", 'WdatePicker('+$this.attr("datePicker")+')');
			if($this.next(".inputDateButton").length == 0){
				var inputDateButton_ = $('<a class="inputDateButton" href="javascript:;">选择</a>');
				$this.after(inputDateButton_);
			}
		});
	}
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			var d__ = unescape($this.attr("data")).replaceTmById($(event.target).parents(".unitBox:first"));
			if (!url.isFinishedTm() || !d__.isFinishedTm()) {
			...
			function nextAction(){
				...
				var d__ = $this.attr("data");
				var businessTypeId = $(":input[name='businessTypeId']",navTab.getCurrentPanel());
				try{
					if(d__)
						d__ = unescape(d__).replaceTmById($(event.target).parents(".unitBox:first"));
					d__ = $.parseJSON(d__);
					if(businessTypeId){
						d__['businessTypeId'] = businessTypeId.val();
					}
				}catch(e){
//					console.info("cannot parse the attr data to json [ " + $this.attr("data")+ " ]");
				}
				d__ = $.fn.extend({pTabid:pTabid}, d__ || {});
				if($this.attr("name"))
					d__['eventSource']=$.base64.encode($this.attr("name"));

				navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external, data:d__});
			 ...
			 */
			if($this.attr('click')){
				var valid = eval("("+$this.attr('click')+")");
				if(valid['url']){//ajax验证
					$.ajax({url:valid['url'],data:valid['data']||{},dataType:valid['dataType']||'text',success:function(data_){
						if(!valid['fun'].call($this,data_)) return false;
						nextAction();
					}});
					return false;
				}else{
					if(!valid['fun'].call($this))return false;
					nextAction();
				}
			}else
				nextAction();
	...
	//dialogs
	$("a[target=dialog],input[target=dialog][type='button']", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
			var d__ = unescape($this.attr("data")).replaceTmById($(event.target).parents(".unitBox:first"));
			...
			function openDialog(){
				var title = $this.val()|| $this.attr("title") || $this.text() ;
				...
				options.eventSource = $this.attr("name");
				var d__ = $this.attr("data");
				var businessTypeId = $(":input[name='businessTypeId']",navTab.getCurrentPanel());
				try{
					if(d__)
						d__ = unescape(d__).replaceTmById($(event.target).parents(".unitBox:first"));
					options.data_ = $.parseJSON(d__);
				}catch(e){
//					console.info($this);
//					console.info("cannot parse the attr data to json [ " + $this.attr("data")+ " ]");
				}
				options.data_ = options.data_ || {};
				if(businessTypeId){
					options.data_['businessTypeId'] = businessTypeId.val();
				}
				if($this.attr("name"))
					options.data_['eventSource']=$.base64.encode($this.attr("name"));
    ...
    $(".subBar li:first", $p).before("<li><label class='' style='vertical-align: bottom;cursor: pointer' onclick='window.open($(\"#old_sys\").attr(\"href\"))'>未找到项目请进入原系统操作</label></li>");
}
...
//		$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:"0px"}, 500);
		//var top_ = ($(window).height()/2 - $(this._boxId).height() * 2) + "px";
		$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:"170px"}, 1);
		if (this._closeTimer) {
	...
	close: function(){
		...
		$(this._boxId).animate({top:-$(this._boxId).height()}, 1, function(){
			$(this).remove();
		});
		$(this._bgId).hide();
	},
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(){
					...
					if (sTarget) {
						...
						$("#"+sTarget, $grid).val($tr.attr("rel"));
						if ($("#selectData"+$table.attr('id'), $grid).size() == 0) {
							$grid.prepend('<input id="selectData'+$table.attr('id')+'" type="hidden" />');
						}
						$("#selectData"+$table.attr('id'), $grid).val($tr.attr('data'));
					}
				});
			});
...
function dwzSearch(form, targetType){
	if(targetType){
		...
		if($.contains($.pdialog.getCurrent(), ($(form))))
			dialogSearch(form);
		else
			navTabSearch(form);
	}
	...
	$.fn.extend({
		lookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					$.pdialog.open(url, $this.attr("lookupGroup"), $this.attr("title") || $this.text(), options);
					return false;
				});
			});
		},
		...
		selectedTodo: function(){
			...
			return this.each(function(){
				...
				var ajaxType_ = $this.attr("method") || "post";
				...
				$this.click(function(){
					...
					function _doPost(){
						$.ajax({
							type:ajaxType_, url:$this.attr('href'), dataType:'json', cache: false,
							data: function(){
								if (postType == 'map'){
